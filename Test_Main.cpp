...
int count_frequency_path(const vector<vector<int>>& matrix,int size){
    int count = 0;
...
vector<vector<int>> random_generated_maze(int size){
    int wall = 1;
    int path = 0;
    ...
    vector<vector<int>> maze_map(maze_size,vector<int>(maze_size,wall)); //Create a 2D array with int 1
...
vector<vector<char>> Turn_To_ASCII(vector<vector<int>> maze_map,int size){
    int maze_size = 2 * size + 1;
    vector<vector<char>> maze_map_in_char(maze_size,vector<char>(maze_size,'1'));
    
    for(int row = 0; row < maze_size ;row++){
        for(int column = 0; column < maze_size;column++){
            if(maze_map[row][column] == 0){
                maze_map_in_char[row][column] = '0';
            }
            else{
                maze_map_in_char[row][column] = '1';
            }
        }
    ...
    for(int row = 0; row < maze_size; row++){
        for(int column = 0; column < maze_size; column++){
            if(row%2 == 0 && maze_map_in_char[row][column] != '0'){
                maze_map_in_char[row][column] = '-';
            }
            else if(row%2 == 1 && maze_map_in_char[row][column] != '0'){
                maze_map_in_char[row][column] = '|';
            }
            else if(((row%2 == 0 && column%2 == 1) || (row%2 == 1 && column%2 == 0)) && maze_map_in_char[row][column] == '0'){
                maze_map_in_char[row][column] = ' ';
            }
...
int main(){
    int size;
    cin>>size;
    vector<vector<int>> res = random_generated_maze(size);
    ...
    vector<vector<char>> res_char = Turn_To_ASCII(res,size);
    for(int i = 0; i < 2*size+1; i++){
        for(int j = 0; j < 2*size+1; j++){
            cout<<res_char[i][j]<<" ";
        }
